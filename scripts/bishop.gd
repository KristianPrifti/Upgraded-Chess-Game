...
func get_possible_moves(position: Vector2, board, turn_player_isWhite):
	...
	while is_in_grid(Vector2(position.x + a, position.y + a)) && (board[position.x + a][position.y + a] == null || board[position.x + a][position.y + a].pass_through):
		moves_to_add.append(Vector2(a, a))
		a += 1
	moves_to_add.append(Vector2(a, a))
	# add moves to the left and down
	while is_in_grid(Vector2(position.x + b, position.y + b)) && (board[position.x + b][position.y + b] == null || board[position.x + b][position.y + b].pass_through):
		moves_to_add.append(Vector2(b, b))
		b -= 1
	moves_to_add.append(Vector2(b, b))
	# add moves to the right and down
	while is_in_grid(Vector2(position.x + c, position.y + d)) && (board[position.x + c][position.y + d] == null || board[position.x + c][position.y + d].pass_through):
		moves_to_add.append(Vector2(c, d))
		c += 1
		d -= 1
	moves_to_add.append(Vector2(c, d))
	# add moves to the left and up
	while is_in_grid(Vector2(position.x + e, position.y + f)) && (board[position.x + e][position.y + f] == null || board[position.x + e][position.y + f].pass_through):
